...
class SliderControlNode(Node):
    def __init__(self):
        ...
        self.slider_values = {i: 0.0 for i in PARAM_JOINTS}  # Используем словарь вместо списка
        self.value_labels = {}
        self.min_labels = {}
        self.max_labels = {}
    ...
    def setup_gui(self):
        try:
            self.root = tk.Tk()
            self.root.title("ROS2 Slider Controller (Humanoid Joints)")
            ...
            for joint_id in PARAM_JOINTS:
                ...
                # Фрейм для ползунка и текущего значения
                slider_value_frame = ttk.Frame(slider_frame)
                slider_value_frame.pack(fill=tk.BOTH, expand=True)
                
                # Ползунок
                slider = ttk.Scale(slider_value_frame,
                                from_=limits[0],
                                to=limits[1],
                                orient=tk.VERTICAL,
                                length=180,
                                command=lambda v, idx=joint_id: self.slider_changed(idx, v))
                slider.set(0.0)
                slider.pack(side=tk.LEFT, fill=tk.Y, expand=True)
                self.sliders[joint_id] = slider
                
                # Текущее значение (центр)
                value_label = ttk.Label(slider_value_frame, 
                                      text="0.0", 
                                      width=8,
                                      font=('Arial', 10, 'bold'),
                                      anchor="center")
            ...
            for r in range((len(PARAM_JOINTS) + 3) // 4):  # Динамическое вычисление строк
    ...
    def slider_changed(self, index, value):
        ...
        self.slider_values[index] = value_float
        
        if index in self.value_labels:
            self.value_labels[index].config(text=f"{value_float:.2f}")
